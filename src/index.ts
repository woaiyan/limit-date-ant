...
const restDate = (day: moment.Moment) => {
    return moment(day)
        .hour(0)
        .minutes(0)
        .seconds(0)
...
const resetByOther = (day1: moment.Moment, day2: moment.Moment) => {
    const other = day2;
    return moment(day1)
        .hours(other.hours())
...
const isOneDay = (day1: moment.Moment, day2: moment.Moment) => {
    return !restDate(day1).isAfter(restDate(day2)) &&
        !restDate(day1).isBefore(restDate(day2));
}

export function useDisableStartDate(endTime: moment.Moment | null, duration: number) {
    return  useCallback((startValue: moment.Moment) => {
...
export function useDisableEndDate(startTime: moment.Moment | null, duration: number) {
    return useCallback((endValue: moment.Moment) => {
...
export function useDisableStartTime(endTime: moment.Moment | null, duration: number) {
    return useCallback((startValue: moment.Moment | null) => {
        if (!startValue) {
            return  {
                disabledHours: () =>  [],
                disabledMinutes: () =>  [],
        ...
        let disabledHours: any = null;
        let disabledMinutes: any = null;
        let disabledSeconds: any = null;
        if (endTime) {
...
export function useDisableEndTime(startTime: moment.Moment | null, duration: number) {
    return useCallback((endValue: moment.Moment | null) => {
        if (!endValue) {
            return  {
                disabledHours: () =>  [],
                disabledMinutes: () =>  [],
